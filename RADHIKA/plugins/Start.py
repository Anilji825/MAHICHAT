...
import psutil
import config
from RADHIKA import _boot_
from RADHIKA import get_readable_time
from RADHIKA import RADHIKA, mongo
...
from RADHIKA.database.chats import get_served_chats, add_served_chat
from RADHIKA.database.users import get_served_users, add_served_user
...
async def set_group_language(chat: Chat):
    ...
    async for message in RADHIKA.get_chat_history(chat.id, limit=50):
...
async def welcomejej(client, message: Message):
    ...
    try:
        for member in message.new_chat_members:
            ...
            if member.id == RADHIKA.id:
                try:
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"sᴇʟᴇᴄᴛ ʟᴀɴɢᴜᴀɢᴇ", callback_data="choose_lang")]])    
                    await message.reply_photo(photo=random.choice(IMG), caption=START.format(RADHIKA.mention or "can't mention", users, chats), reply_markup=reply_markup)
                    chat = message.chat
                    ...
                    pass
                try:
                    invitelink = await RADHIKA.export_chat_invite_link(message.chat.id)
                ...
                try:
                    groups_photo = await RADHIKA.download_media(
                ...
                count = await RADHIKA.get_chat_members_count(chat.id)
                chats = len(await get_served_chats())
...
async def start(_, m: Message):
    ...
    if m.chat.type == ChatType.PRIVATE:
        ...
        if m.chat.photo:
            try:
                userss_photo = await RADHIKA.download_media(m.chat.photo.big_file_id)
                await umm.delete()
                if userss_photo:
        ...
        await m.reply_photo(photo=chat_photo, caption=START.format(RADHIKA.mention or "can't mention", users, chats, UP), reply_markup=InlineKeyboardMarkup(START_BOT))
        await add_served_user(m.chat.id)
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(f"{m.chat.first_name}", user_id=m.chat.id)]])
        await RADHIKA.send_photo(int(OWNER_ID), photo=chat_photo, caption=f"{m.from_user.mention} ʜᴀs sᴛᴀʀᴛᴇᴅ ʙᴏᴛ. \n\n**ɴᴀᴍᴇ :** {m.chat.first_name}\n**ᴜsᴇʀɴᴀᴍᴇ :** @{m.chat.username}\n**ɪᴅ :** {m.chat.id}\n\n**ᴛᴏᴛᴀʟ ᴜsᴇʀs :** {users}", reply_markup=keyboard)
...
@RADHIKA.on_cmd("help")
async def help(client: RADHIKA, m: Message):
    if m.chat.type == ChatType.PRIVATE:
...
async def ping(_, message: Message):
    ...
    await loda.edit_text(
        text=f"нey вαву!!\n{RADHIKA.name} ᴄʜᴀᴛʙᴏᴛ ιѕ alιve 🥀 αnd worĸιng ғιne wιтн a pιng oғ\n\n**➥** `{ms}` ms\n**➲ ᴄᴘᴜ:** {CPU}\n**➲ ʀᴀᴍ:** {RAM}\n**➲ ᴅɪsᴋ:** {DISK}\n**➲ ᴜᴘᴛɪᴍᴇ »** {UP}\n\n<b>||**๏ мαdє ωιтн ❣️ ву [ʀᴀᴊᴀ ʙᴀʙᴜ](https://t.me/{OWNER_USERNAME}) **||</b>",
...
@RADHIKA.on_message(filters.command("stats"))
                ...
                for chat in chats:
                    ...
                    try:
                        if broadcast_type == "reply":
                            m = await RADHIKA.forward_messages(
                                chat_id, message.chat.id, [broadcast_content.id]
                            )
                        else:
                            m = await RADHIKA.send_message(
